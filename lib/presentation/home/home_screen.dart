      ...
      body: DoubleBack(
        ...
        child: SingleChildScrollView(
          child: Column(
            ...
            children: [
              _buildUserInfo(controller, context),
              _buildMainShortCuts(),
              _buildSecondaryShortCuts(controller),
              _buildDrafts(controller),
              _buildNoInvoiceCustomer(controller),
              _buildInvoices(controller),
              _buildCashflow(),
            ],
          ),
        ),
...
Widget _buildUserInfo(HomeController controller, BuildContext context) {
  return Obx(() {
...
_buildSecondaryShortCuts(HomeController controller) {
  return SizedBox(
    height: 150,
    child: Card(
      child: Row(
...
_buildInvoices(HomeController controller) {
  return ValueListenableBuilder(
    valueListenable: controller.storage.invoicesListenable(),
    builder: (context, box, widget) {
      var openInvoices = box.values
          .where((invoice) =>
...
_buildDrafts(HomeController controller) {
  return ValueListenableBuilder(
    valueListenable: controller.storage.invoicesListenable(),
    builder: (context, box, widget) {
      int drafts = box.values.where((invoice) => invoice.status == "0").length;
...
_buildNoInvoiceCustomer(HomeController controller) {
  return ValueListenableBuilder(
    valueListenable: controller.storage.customersListenable(),
    builder: (context, box, widget) {
      ...
      return noInvoiceCustomers.isEmpty
          ? SizedBox()
          : Card(
              child: ListTile(
